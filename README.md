...
標準出力の状態判定、数値のビット配列表示、数値計算を考えるサンプルコード。
...
- `divisible.c`
  2つの正の整数を受け取り、1つ目が2つ目で割り切れるかを終了ステータスで返す C プログラムです。
  エラー時は `LC_ALL` / `LC_MESSAGES` / `LANG` に応じて英語または日本語のメッセージを出力します。

...
double newton( double v, double x0, double threshold, int maxloop ); //前方宣言
...
int main( void )
{
    ...
    for ( v=0.0/1000; v<20.0/1000+dv/2; v+= dv ) {
        x = newton( v, 1e+12, 1e-16, 500 ); //滴定量v、初期値、収束判定のしきい値、繰返し最大回数
        printf( "v= %.4lf L : solution = %25.15le %6.3lf\n", v, x, -log10(x) );
    }
    return 0;
}

double newton( double v, double x0, double threshold, int maxloop )
{
    int     i;
    double  x, dx;
...
//    printf( "#v=%.15le\n", v );
//    printf( "#%4s : %25s  %25s\n", "iter", "x", "dx" );
    for ( x=x0, i=0; i < maxloop; i++ ) {
        dx = - f(x,v) / df(x,v);
        x = x + dx;
//        printf( " %4d : %25.15le  %25.15le\n", i, x, dx );
        if ( fabs( dx ) < threshold ) break;
    }
    printf( " %4d : %25.15le  %25.15le    ", i, x, dx );
    if ( i == maxloop ) {
...
//    return (v0+v)*x*x + ( (v0+v)*ka + m*v )*x + (m*v-m0*v0)*ka - (v0+v)*kw - (v0+v)*kw*ka/x;
...
{
    double  v0=10.0/1000, m0=0.1, m=0.1, ka=1.74e-5, kw=1e-14;
//    return 2.0*(v0+v)*x + ( (v0+v)*ka + m*v ) + (v0+v)*kw*ka/x/x;
